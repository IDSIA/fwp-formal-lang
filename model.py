...
from layers import (
    FastFFlayer, RecUpdateTanhFastFFlayer,
    LinearAttentionlayer, Attentionlayer, SRWMlayer)
...
class TrafoModel(BaseModel):
    def __init__(self, hidden_size, in_vocab_size, out_vocab_size, dropout=0.0,
                 nheads=8, num_layers=6, ff_factor=4, use_pos_enc=False):
        super().__init__()
        self.in_vocab_size = in_vocab_size
        self.out_vocab_size = out_vocab_size
        self.hidden_size = hidden_size
...
# SRWM with delta rule.
class SRWM(BaseModel):
    def __init__(self, in_vocab_size, out_vocab_size, hidden_size,
                 num_layers, num_head, dim_head, dim_ff,
                 dropout, use_pos_enc=False):
        super(SRWM, self).__init__()
        assert num_head * dim_head == hidden_size
        ...
        for _ in range(num_layers):  # each "layer" consists of two sub-layers
            layers.append(
                SRWMlayer(num_head, dim_head, hidden_size, dropout))
            layers.append(
                TransformerFFlayers(dim_ff, hidden_size, dropout))
        self.layers = nn.Sequential(*layers)
        self.out_layer = nn.Linear(hidden_size, out_vocab_size)
...
# Linear Transformer with delta update rule.
class DeltaNet(BaseModel):
    def __init__(self, in_vocab_size, out_vocab_size, hidden_size,
                 num_layers, num_head, dim_head, dim_ff,
                 dropout, use_pos_enc=False):
...
class RecurrentDelta(BaseModel):
    def __init__(self, in_vocab_size, out_vocab_size, hidden_size,
                 num_layers, num_head, dim_head, dim_ff,
                 dropout, use_pos_enc=False):
        super(RecurrentDelta, self).__init__()
        assert num_head * dim_head == hidden_size
...
if __name__ == '__main__':
    ...
    print("========================")
    print(f"  Test FastFFRecUpdateTanh  {datetime.now()}")
    print("========================")

    model = FastFFRecUpdateTanh(in_vocab_size=in_vocab_size,
                                out_vocab_size=out_vocab_size,
                                hidden_size=hidden_size,
                                num_layers=num_layers,
                                num_head=nheads, dim_head=dim_head,
